(function(e,t,n,r){
	function u(t,n){
		...
		this._invalidated={};
		this._pipe=[];
		e.each(u.Plugins,e.proxy(function(e,t){this._plugins[e[0].toLowerCase()+e.slice(1)]=new t(this)},this));
		e.each(u.Pipe,e.proxy(function(t,n){this._pipe.push({filter:n.filter,run:e.proxy(n.run,this)})},this));
		this.setup();
		this.initialize()}
		...
		var i,s,o;i={
			...
			s={
				isTouch:false,isScrolling:false,isSwiping:false,direction:false,inMotion:false
			};
			o={
			...
			u.Plugins={};
			u.Pipe=[{
				filter:["width","items","settings"],run:function(e){e.current=this._items&&this._items[this.relative(this._current)]}
			},{
			...
			{filter:["items","settings"],run:function(){
				var e,t,n=this._clones,r=this._items,i=this.settings.loop?n.length-Math.max(this.settings.items*2,4):0;for(e=0,t=Math.abs(i/2);e<t;e++){
					if(i>0){
		...
		},
		{
			filter:["width","items","settings"],run:function(){
				...
				this._coordinates=[];
				for(i=0,s=this._clones.length+this._items.length;
					i<s;i++)
				{
					r=this._mergers[this.relative(i)];
					r=this.settings.mergeFit&&Math.min(r,this.settings.items)||r;
					n+=(this.settings.autoWidth?this._items[this.relative(i)].width()+this.settings.margin:t*r)*e;this._coordinates.push(n)
				}
			}
		},
		{
			filter:["width","items","settings"],run:function(){
				var t,n,r=(this.width()/this.settings.items).toFixed(3),i={
					width:Math.abs(this._coordinates[this._coordinates.length-1])+this.settings.stagePadding*2,"padding-left":this.settings.stagePadding||"","padding-right":this.settings.stagePadding||""
				};
				this.$stage.css(i);
				i={
				...
				if(!this.settings.autoWidth&&e.grep(this._mergers,function(e){
					return e>1
				}).length>0){for(t=0,n=this._coordinates.length;
					t<n;t++){
					i.width=Math.abs(this._coordinates[t])-Math.abs(this._coordinates[t-1]||0)-this.settings.margin;this.$stage.children().eq(t).css(i)}
				}else{
		...
		},
		{
			filter:["width","position","items","settings"],run:function(){
				...
				for(u=0,a=this._coordinates.length;
					u<a;u++){
			...
			u.prototype.setup=function(){
				...
				u.prototype.prepare=function(t){
					...
					u.prototype.width=function(e){
						e=e||u.Width.Default;switch(e){
							case u.Width.Inner:case u.Width.Outer:return this._width;
					...
					u.prototype.refresh=function(){
						...
						u.prototype.eventsCall=function(){
							...
							this.e._onDragEnd=e.proxy(function(e){this.onDragEnd(e)},this);
							this.e._onResize=e.proxy(function(e){this.onResize(e)},this);
							this.e._transitionEnd=e.proxy(function(e){this.transitionEnd(e)},this);
							this.e._preventClick=e.proxy(function(e){this.preventClick(e)},this)};
							u.prototype.onThrottledResize=function(){
								...
								u.prototype.onResize=function(){
									...
									u.prototype.eventsRouter=function(e){
										var t=e.type;
										if(t==="mousedown"||t==="touchstart"){this.onDragStart(e)
										}else if(t==="mousemove"||t==="touchmove"){
											this.onDragMove(e)
										}else if(t==="mouseup"||t==="touchend"){
											this.onDragEnd(e)}
											else if(t==="touchcancel"){this.onDragEnd(e)
											}
										};
										...
										u.prototype.onDragStart=function(r){
											...
											this.drag.targetEl=i.target||i.srcElement;this.drag.updatedX=this.drag.start;
